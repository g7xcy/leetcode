...
;; count-less-num :: SegmentTree -> Integer -> Integer
(define (count-less-num st prefix)
  (match prefix
    [0 0]
    [x (segment-tree-query st 0 (sub1 x))]))

;; count-majority-subarrays* :: SegmentTree -> (Integer -> Integer) -> Integer -> List Integer -> Integer
(define (count-majority-subarrays* st prefix->index prefix xs)
  (let rec ([xs xs]
            [index (prefix->index prefix)])
    (let ([less-num (count-less-num st index)])
      (segment-tree-update-with! st index add1)
      (match xs
        ['() less-num]
        [(cons x rest)
         (+ less-num
...
(define/contract (count-majority-subarrays nums target)
  ...
  (let* ([len (length nums)]
         [xs (map (lambda (x) (if (= x target) 1 -1)) nums)]
         [vec (make-vector (add1 (* 2 len)))]
         [st (make-segment-tree vec identity +)])
    (count-majority-subarrays* st (curry + len) 0 xs)))
